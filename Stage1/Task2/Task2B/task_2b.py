...
def classify_event(image):
    ...
    ADD YOUR CODE HERE
    '''
    #D:\MNNIT\eyantra\Geo_Guide_3344_Robot_Raiders\Task2\Task2B\CNNmodel_transfer_learning.h5
    CNN_model = load_model(os.path.join('','CNNmodel_transfer_learning.h5'),custom_objects={'KerasLayer':hub.KerasLayer})
    img=cv2.imread(image)
    resize = tf.image.resize(img, (224,224))
    event_key=np.argmax(CNN_model.predict(np.expand_dims(resize/255, 0)))
    
    
    event = event_name(event_key)  #"variable to return the detected function"
    return event
