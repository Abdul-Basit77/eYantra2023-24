...
def check_events2(roi):
    ...
    green_mask = (image[:, :, 1] >= 75) & (image[:, :, 1] <= 130)
...
def detect_events(transformed_frame):
    ...
    event_coordinates=[]
    for cnt in contours:
        if (5800 < cv2.contourArea(cnt) < 8000):
            ...
            if w/h>0.9 and w/h<1.1:
                ...
                cv2.rectangle(transformed_frame, (x+7, y+7), (x + w-7, y + h-7), (0, 255, 0), 2)
...
if __name__ == "__main__":
    ...
    import pandas as pd
    (pd.DataFrame.from_dict(data=identified_labels, orient='index')
